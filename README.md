...
| Stage | Module | Output |
|---|---|---|
| Compose config | `configs/` + `cotobf.configs` | resolved `config.yaml` |
| Build data | `cotobf.data` | splits + manifest |
| Load model | `cotobf.models` | `LoadedModel` + resolved commit |
| Run stages | `scripts/run_experiment.py` | per-stage JSON |
| Aggregate | `cotobf.reporting` | `results.json` + tables + figures |
...
├── src/cotobf/       # installable library (print-free)
...
Ablations live under `src/cotobf/ablation/` and return structured dicts. Presets
