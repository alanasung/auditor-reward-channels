...
|---|---|---|
| Config schema | `cotobf.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `cotobf.configs.loader` | Hydra compose API |
| Device | `cotobf.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `cotobf.models.registry` | Pinned revisions + hardware fit |
| Hooks | `cotobf.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `cotobf.models.generation` | Chat template path recording |
| Data | `cotobf.data` | Synthetic items, splits, manifests |
| Evaluation | `cotobf.evaluation` | Metrics, failure harness, protocols |
| Ablation | `cotobf.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `cotobf.reporting` | Aggregate → MD/TeX/figures |
| Cache | `cotobf.cache` | Versioned artifact store |
...
|---|---:|---|
| local M4 pilot | ~32 (`power_status=micro`) | Finishes on Apple Silicon; not powered for headline nulls |
...
`src/cotobf/cotobf/` are preserved across regeneration.
...
Every reward-shape arm is scored with a **single fixed held-out monitor**.
Arm-own-monitor comparisons are forbidden.

### Entailment scorer

Thought/output differentiation uses entailment (does CoT entail the action?),
validated to score obfuscated cheats lower than confessed/honest solutions.

### TRL / GRPO

Prefer pinned `trl==0.14.0`. GRPO API is probed at runtime with synthetic
fallback and explicit `trl_status`. Expert iteration is harness smoke only.
